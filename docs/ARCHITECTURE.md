...
| **Gate expiry** | No decision in 7 days | none | Sweep job closes it: `status=rejected`, reason `gate_expired`. **Still not built** — and deliberately out of reach of the block C sweep, whose candidate statuses are `queued` and `running` only ([ADR 0021](adr/0021-stale-job-reconciliation-and-dlq-recovery.md) decision 2) |
| **A hard kill leaves a non-terminal row and a dead-lettered message** | An operator sweep: `queued`/`running`, `completed_at IS NULL`, no durable activity for `STALE_JOB_MIN_AGE_SECONDS` | none — the message is already gone | **Repair before failure** under the same per-job execution fence a worker takes: a pending interrupt restores `awaiting_approval`, a terminal checkpoint writes the terminal row, and only a message in the dead-letter queue makes it `failed` with `job_dead_lettered`. Ambiguous evidence is `skipped`. **Age selects a candidate and never authorises a change** (ADR 0021) |
| **A job whose enqueue failed sits `queued` with no message** | The same sweep: `queued`, no checkpoint at all, nothing in the dead-letter queue | none | Re-enqueued with the job's own `idempotency_key` — the message `POST /jobs` would have sent (ADR 0010 decision 10's other end) |
...
| 38 | **[ADR 0021]** Operational recovery is **three operator-run scripts**, fenced by ADR 0016's advisory lock, deciding from durable state. Age selects a candidate and never authorises a mutation; a job is failed only with a dead-lettered message as evidence; a replay preserves the original `MessageGroupId` and `MessageDeduplicationId`; and CloudWatch alarms read native metrics only | An automatic reconciler would need its own ownership notion, its own alarm and its own failure mode — and a blind DLQ redrive repeats the outage that filled it. A person reading a dry run is the control | A stale row can sit `skipped` until somebody looks. `RunningTaskCount` alarms are given up with Container Insights, so worker liveness is inferred from queue age |
