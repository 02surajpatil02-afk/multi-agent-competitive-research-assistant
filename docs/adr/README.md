...
| [0018](0018-the-ci-evaluation-gate-protects-the-contract-not-the-quality.md) | The CI evaluation gate is a **regression contract over committed fixtures, not a quality threshold**: no percentage anywhere. Each case declares `expect_failing_metrics` and must fail exactly those; a defect that stops being caught fails the build. Three exit codes separate a violation from an unusable report. The judge stays out of PR CI behind a `workflow_dispatch` workflow whose only failure is "scored nothing at all". No Prometheus, no Grafana, no HOLDOUT | Accepted, **built** (Phase 4 block C) | 2026-08-19 |
| [0019](0019-no-nat-gateway-in-the-temporary-aws-deployment.md) | The temporary AWS deployment runs both Fargate services in **public subnets with public IPs and no NAT gateway**; the security group is the control, and the worker has no ingress rule at all. RDS and ElastiCache stay in subnets with no route off the VPC and accept only group-referenced traffic. Two AZs because the ALB and the subnet groups require two - not as a redundancy claim. The long-running production shape is documented rather than built | Accepted, **built** (Phase 5 block A); **never applied** | 2026-08-20 |
